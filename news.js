...
class LiveNewsHub {
    ...
    initializeDOMElements() {
        ...
        // Premium elements
        this.premiumModal = new bootstrap.Modal(document.getElementById('premiumModal'));
        this.loginModal = new bootstrap.Modal(document.getElementById('loginModal'));
        this.subscriptionSuccessModal = new bootstrap.Modal(document.getElementById('subscriptionSuccessModal'));
    }

    // User authentication and premium features
    checkLoginStatus() {
        const userData = localStorage.getItem('newsHub_user');
        if (userData) {
    ...
    checkSubscriptionStatus() {
        const subscriptionData = localStorage.getItem('newsHub_subscription');
        if (subscriptionData) {
            ...
            if (now < expiryDate) {
                ...
                if (this.user) {
                    ...
                    localStorage.setItem('newsHub_user', JSON.stringify(this.user));
                }
                this.updateUI();
            } else {
                ...
                localStorage.removeItem('newsHub_subscription');
                this.updateUI();
    ...
    updateUI() {
        ...
        const userAvatar = document.getElementById('userMenuButton');
        ...
        const customizeUpgradeBtn = document.getElementById('customizeUpgradeBtn');
        const savedUpgradeBtn = document.getElementById('savedUpgradeBtn');
        ...
        if (this.user) {
            ...
            userAvatar.innerHTML = this.user.name.charAt(0).toUpperCase();

            if (this.isPremium) {
                userStatus.innerHTML = '<i class="bi bi-star-fill" style="color: var(--premium-color);"></i> Premium Account';
                if (adContainer) adContainer.style.display = 'none';
                if (personalizationPanel) personalizationPanel.classList.remove('premium-only');
                if (savedArticlesContainer) savedArticlesContainer.style.display = 'block';
                if (aiSuggestionsContainer) aiSuggestionsContainer.style.display = 'block';
                if (premiumMenuItem) premiumMenuItem.style.display = 'none';
                if (customizeUpgradeBtn) customizeUpgradeBtn.style.display = 'none';
                if (savedUpgradeBtn) savedUpgradeBtn.style.display = 'none';
                if (adUpgradeBtn) adUpgradeBtn.style.display = 'none';
                ...
                document.querySelectorAll('.personalization-option input').forEach(input => {
                ...
                // Enable premium content buttons
                this.enablePremiumContentButtons();
            } else {
                userStatus.innerHTML = '<i class="bi bi-circle-fill text-danger"></i> Free Account';
                if (adContainer) adContainer.style.display = 'block';
                if (personalizationPanel) personalizationPanel.classList.add('premium-only');
                if (savedArticlesContainer) savedArticlesContainer.style.display = 'none';
                if (aiSuggestionsContainer) aiSuggestionsContainer.style.display = 'none';
                if (premiumMenuItem) premiumMenuItem.style.display = 'block';
                if (customizeUpgradeBtn) customizeUpgradeBtn.style.display = 'block';
                if (savedUpgradeBtn) savedUpgradeBtn.style.display = 'block';
                if (adUpgradeBtn) adUpgradeBtn.style.display = 'block';
                ...
                // Disable premium content buttons
                this.disablePremiumContentButtons();
            }
        } else {
            ...
            userStatus.innerHTML = '<i class="bi bi-circle-fill text-danger"></i> Free Account';
            userAvatar.innerHTML = '<i class="bi bi-person"></i>';
    ...
    enablePremiumContentButtons() {
        const premiumCards = document.querySelectorAll('#premiumContentContainer .news-card');
        premiumCards.forEach(card => {
            const previewBtn = card.querySelector('.btn-preview');
            const readBtn = card.querySelector('.btn-read');
            
            if (previewBtn) {
                previewBtn.onclick = () => this.openPremiumPreview(card);
            }
            
            if (readBtn) {
                readBtn.onclick = () => this.openPremiumArticle(card);
            }
        });
    }

    disablePremiumContentButtons() {
        const premiumCards = document.querySelectorAll('#premiumContentContainer .news-card');
        premiumCards.forEach(card => {
            const previewBtn = card.querySelector('.btn-preview');
            const readBtn = card.querySelector('.btn-read');
            ...
            if (previewBtn) {
                previewBtn.onclick = () => this.promptPremium();
            }
            
            if (readBtn) {
                readBtn.onclick = () => this.promptPremium();
            }
        });
    }

    openPremiumPreview(card) {
        const title = card.querySelector('.card-title').textContent;
        const description = card.querySelector('.card-text').textContent;
        const image = card.querySelector('.news-card-img').src;
    ...
    openPremiumArticle(card) {
        const title = card.querySelector('.card-title').textContent;
        const description = card.querySelector('.card-text').textContent;
        
        this.displayFullContentModal(
            title,
            `<p>${description}</p><p>This is exclusive premium content available only to subscribers. Enjoy in-depth analysis and expert insights not available to free users.</p><p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam euismod, nisl eget ultricies ultricies, nunc nisl aliquam nunc, eget aliquam nisl nunc eget nisl. Nullam euismod, nisl eget ultricies ultricies, nunc nisl aliquam nunc, eget aliquam nisl nunc eget nisl.</p><p>Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo.</p>`,
            '#premium-content'
        );
    }
    ...
    subscribeToPremium() {
        ...
        localStorage.setItem('newsHub_subscription', JSON.stringify(subscriptionData));
        ...
        if (this.user) {
            ...
            // Create a guest user with premium
            this.user = {
                name: 'Premium User',
        ...
        // Close premium modal and show success
        this.premiumModal.hide();

        // Show subscription details in success modal
        document.getElementById('subscriptionPlan').textContent = 'Premium Monthly';
        document.getElementById('subscriptionStart').textContent = new Date().toLocaleDateString();
        document.getElementById('subscriptionEnd').textContent = expiryDate.toLocaleDateString();

        // Show success modal
        setTimeout(() => {
            this.subscriptionSuccessModal.show();
        }, 300);

        this.showNotification('Premium subscription activated!', 'success');

        // Start premium content refresh timer
        this.setupPremiumContentRefresh();
    }
    ...
    showSignupModal() {
        this.loginModal.hide();
        this.showNotification('Sign up functionality would be implemented here', 'info');
    }

    promptPremium() {
        this.showPremiumModal();
        this.showNotification('This content is available to Premium subscribers only', 'warning');
    }

    setupPremiumContentRefresh() {
        // Refresh premium content every 30 minutes
        if (this.premiumRefreshInterval) {
            clearInterval(this.premiumRefreshInterval);
        }

        this.premiumRefreshInterval = setInterval(() => {
            if (this.isPremium) {
                this.showNotification('Refreshing premium content...', 'info');
                this.refreshPremiumContent();
            } else {
                clearInterval(this.premiumRefreshInterval);
            }
    ...
    refreshPremiumContent() {
        // Simulate premium content refresh
        const premiumCards = document.querySelectorAll('#premiumContentContainer .news-card');
        premiumCards.forEach(card => {
        ...
        this.showNotification('Premium content refreshed', 'success');
    }
    ...
    loadSavedArticles() {
        ...
        if (savedArticles.length === 0) {
            savedArticlesList.innerHTML = '<p class="text-center text-muted">No saved articles yet.</p>';
            return;
        }

        savedArticlesList.innerHTML = savedArticles.map((article, index) => `
            <div class="saved-article">
                <div class="saved-article-title">${this.truncateText(article.title, 60)}</div>
                <div class="saved-article-actions">
                    <button class="btn btn-sm btn-outline-primary" onclick="newsHub.readFullArticle('${article.url}')">
                        <i class="bi bi-eye"></i>
                    </button>
                    <button class="btn btn-sm btn-outline-danger" onclick="newsHub.removeSavedArticle(${index})">
    ...
    removeSavedArticle(index) {
        const savedArticles = JSON.parse(localStorage.getItem('saved_articles') || '[]');
        savedArticles.splice(index, 1);
        localStorage.setItem('saved_articles', JSON.stringify(savedArticles));
        this.loadSavedArticles();
        this.showNotification('Article removed from saved items', 'success');
    }

    generateAISuggestions() {
        const suggestions = [
            "Based on your reading history, you might enjoy these AI technology articles",
            "Trending in business: Latest market analysis and financial news",
            "Popular science stories this week that match your interests"
        ];
        ...
        aiSuggestionsList.innerHTML = suggestions.map(suggestion => `
            <div class="ai-suggestion">
                <span class="ai-suggestion-badge">AI Recommendation</span>
                ${suggestion}
            </div>
        `).join('');
    }
    ...
    async createYouTubeEmbed(videoId, containerId) {
        ...
        try {
            ...
            if (isValid) {
                // ✅ Show the video
                container.innerHTML = `
                    <iframe width="100%" height="400" src="https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&controls=1&modestbranding=1&rel=0" frameborder="0" allowfullscreen allow="autoplay; encrypted-media">
                    </iframe>
                `;
                return true;
            } else {
    ...
    setupAutoRefresh() {
        ...
        // Setup premium content refresh if user is premium
        if (this.isPremium) {
            this.setupPremiumContentRefresh();
        }
    }
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <div class="mt-auto">
                        <div class="action-buttons">
                            ...
                            ` : ''}
                            <button class="favorite-btn" onclick="newsHub.toggleFavorite(this)" title="Save article">
    ...
    analyzeSentiment(text) {
        ...
        const positiveWords = ['good', 'great', 'excellent', 'amazing', 'positive', 'success', 'win', 'growth', 'profit'];
        const negativeWords = ['bad', 'terrible', 'awful', 'negative', 'failure', 'loss', 'decline', 'crisis', 'war'];
    ...
    toggleFavorite(button) {
        ...
        const articleData = this.getArticleDataFromCard(card);
        ...
        if (button.classList.contains('active')) {
            ...
            if (!isAlreadySaved) {
                ...
                localStorage.setItem('saved_articles', JSON.stringify(savedArticles));
                this.showNotification('Article saved to favorites', 'success');
            }
        } else {
            // Remove from saved articles
            const index = savedArticles.findIndex(article => article.url === articleData.url);
            
            if (index !== -1) {
                savedArticles.splice(index, 1);
                localStorage.setItem('saved_articles', JSON.stringify(savedArticles));
                this.showNotification('Article removed from favorites', 'info');
            }
        }
        
        // Refresh saved articles list if visible
        if (this.isPremium) {
            this.loadSavedArticles();
        }
    }
    ...
    async loadVideoContent(videoId, videoType) {
        ...
        try {
            if (videoType === 'youtube' && videoId) {
                ...
                // Vimeo embed
                container.innerHTML = `
                    <iframe class="video-player-centered" src="https://player.vimeo.com/video/148751763?autoplay=1&muted=1" frameborder="0" allowfullscreen>
                    </iframe>
                `;
